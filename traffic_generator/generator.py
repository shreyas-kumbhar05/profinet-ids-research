...
def run_generator(config):
    """
    Main loop. Sends PROFINET RT frames with compensated timing.
    Compensated timing prevents drift accumulation over 150,000 frames.
    """
    t   = config["timing"]
    c   = config["capture"]
    g   = config["generator"]
    ...
    # Convert to nanoseconds for high-resolution timing
...
# Open one persistent Layer 2 socket
    ...
    try:
        while True:
            ...
            # Compensated timing — next send relative to start,
            # not previous frame — prevents drift accumulation
...
def parse_args():
    p = argparse.ArgumentParser(description="PROFINET RT traffic generator")
    p.add_argument("--config",      default="traffic_generator/config.yaml")
    p.add_argument("--cycle-time",  type=float, help="Cycle time in ms")
